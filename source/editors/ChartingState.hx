...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Default Cam Zoom', "Quick and easy default camera zoom change,\nthe game already tweens to the new value\n\nValue 1: New zoom value"],
		...
		['Camera Visibility', "Camera Alpha fuckery\n\nValue 1: Value to fade to, fade time. Formatted as: \"1, 0.5\". \nValue 2: Camera, can be Game or HUD"],
		['Character Visibility', "Character Alpha fuckery\n\nValue 1: Value to fade to, fade time. Formatted as: \"1, 0.5\". \nValue 2: Character"],
		['Screen Flash', "Value 1: Camera flash\nValue 2: HUD flash\n\nThe number for each value is the duration."],
		['Stalkers Walk In', "No variables, only used on the idiot stage\nto make the stalkers tween in do not use on other stages!!!!1"],
		['Background Flash', "First value is for the time it takes to fade,\nonly used on the idiot stage to make the background flash ALSO do not use on other stages!!!!1"],
		['Idiot Text Scroll', "No vars, scrolls a few random texts back across the back"],
		['Idiot Bluescreen', "The Idiot end Bluescreen. No vars just does the do"],
		['Idiot Invert', "Inverts camGame and some hud features.\nValue 1 is ON or OFF"],
		['Tween Window Size', "Tweens the entire window size, only works when windowed.\nValue 1: Width, Height (set at 0 for no difference)\nValue 2: Time"],
		['Tween Window Position', "Tweens the entire window size, only works when windowed.\nValue 1: X, Y (set at 0 for no difference)\nValue 2: Time"],
		['Create Idiot Popup', "lol"],
		['Cereal Spit', "Cereal Guy spittin shit\nValue 1: Health lost on spit"],
		['Cereal Table Flip End', "Cereal Guy end table throwing function, no vars"],
		['Change Icon', "Used on songs with gf singing in place of the enemy,\nstupid way to just change the Icon\nValue 1: Character Icon to change\nValue 2: New Icon name"],
		['Set Haxe Shader', "Value 1: Camera to set shader on\nValue 2: Shader"],
		['Chromatic Bump', "Value 1: Peak value, ending value, decay time (split with commas like so)\nValue 2: Camera to set chromatic on"],
		['Display Credit', "No associated values, just displays the Song Credit again"],
		['Set Cam On Character', "Change the camera focus to a specific character\nValue 1: Character"],
		['Weegee Video Controls', "Kinda just three things, if yk yk\nValue 1: 'play', 'reveal', or 'remove'"],
		['HealthBar InOut', "kms"],
		['Countdown', "Manually creates a countdown sprite and plays the related sound\nValue 1: The Countdown number\n 3 = 3 (no sprite), 2 = 2 ready, 1 = 1 set, 0 = 0 go\nValue 2: Countdown path suffix"],
		['Set New Note Keys', "Sets new keybinds dependent on Mania count, \nwill NOT update anything but (chart mania, the notes in-game, ect will not change)\nValue 1: New Mania count (works the same as the charting variable)\nValue 2: Show letters under notes for new keybinds, can be true or false"],
		['Add Spacebar Button', "Used in Obey and Absurde, adds the little popup telling\nyou when to mash spacebar for the spiral mechanic.\nValue 1: Time it takes until it fades offscreen"],
		['We\'re Not Done Here Yet.', "Used in that one part in foretoken youcan\nuse it anywhere but like why would you looooool\n(its like 1 am when im typing this idk if\nill even remember putting this here"],
	...
	**/
	...
	override function create()
	{
		...
		for (i in 0...tipTextArray.length) {
			...
			tipText.y += i * 12;
			tipText.setFormat(Paths.font("impact.otf"), 14, FlxColor.WHITE, LEFT/*, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK*/);
	...
	var creditInput:FlxUIInputText;
	var cardDropDown:FlxUIDropDownMenuCustom;
	...
	function addSongUI():Void
	{
		...
		var stepperMania:FlxUINumericStepper = new FlxUINumericStepper(100, stepperBPM.y + 35, 1, 0, 0, 3, 1);
		...
		creditInput = new FlxUIInputText(stageDropDown.x, stageDropDown.y + 40, 120, _song.credit, 8);
		blockPressWhileTypingOn.push(creditInput);



		#if MODS_ALLOWED
		//using getPreloadPath is stupid but im lazy and it works so whatevs
		//dont ask why this technically has mod support idfk
		var directories:Array<String> = [Paths.mods('images/titleCards/'), Paths.mods(Paths.currentModDirectory + '/images/titleCards/'), Paths.getPreloadPath('shared/images/titleCards/')];
		for(mod in Paths.getGlobalMods())
			directories.push(Paths.mods(mod + '/images/titleCards/'));
		#else
		var directories:Array<String> = [Paths.getPreloadPath('shared/images/titleCards/')];
		#end
		...
		for (i in 0...directories.length) 
		{
			...
			if(FileSystem.exists(directory)) 
			{
				for (file in FileSystem.readDirectory(directory)) 
				{
					...
					{
						var cardToCheck:String = file.substr(0, file.length - 4);
						if(!tempMap.exists(cardToCheck)) 
						{
							tempMap.set(cardToCheck, true);
							cards.push(cardToCheck);
						}
					}
				}
			}
		...
		#end
		if(cards.length < 1) cards.push('awesome');

		cardDropDown = new FlxUIDropDownMenuCustom(stageDropDown.x, creditInput.y + 40, FlxUIDropDownMenuCustom.makeStrIdLabelArray(cards, true), function(character:String)
		{
			_song.card = cards[Std.parseInt(character)];
		});
		cardDropDown.selectedLabel = _song.card;
		blockPressWhileScrolling.push(cardDropDown);
		...
		tab_group_song.add(new FlxText(creditInput.x, creditInput.y - 15, 0, 'Credit:'));
		tab_group_song.add(new FlxText(cardDropDown.x, cardDropDown.y - 15, 0, 'Card Image:'));
		...
		tab_group_song.add(creditInput);
		tab_group_song.add(cardDropDown);
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			{
				autosaveSong();
				//for now im just removing this cause i cant be bothered, ill fix it up for next update maybe
		...
		curRenderedNotes.forEachAlive(function(note:Note) {
			...
			if(note.strumTime <= Conductor.songPosition && note.noteData < 4) {
				note.alpha = 0.4;
	...
	*/
	...
	function reloadGridLayer() {
		gridLayer.clear();
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE + GRID_SIZE * Main.ammo[_song.mania] * 2, Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]));
		...
		{
			nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE + GRID_SIZE * Main.ammo[_song.mania] * 2, Std.int(GRID_SIZE * getSectionBeats(curSec + 1) * 4 * zoomList[curZoom]));
		...
		if(foundNextSec)
		{
			var gridBlack:FlxSprite = new FlxSprite(0, gridBG.height).makeGraphic(Std.int(GRID_SIZE + GRID_SIZE * Main.ammo[_song.mania] * 2), Std.int(nextGridBG.height), FlxColor.BLACK);
			gridBlack.alpha = 0.4;
			gridLayer.add(gridBlack);
		}

		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE * Main.ammo[_song.mania])).makeGraphic(2, leHeight, FlxColor.BLACK);
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[3] != null && note.noteType != null && note.noteType.length > 0) {
				...
				daText.setFormat(Paths.font("impact.otf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				daText.xAdd = -32;
				daText.yAdd = 6;
				daText.borderSize = 1;
		...
		for (i in _song.events)
		{
			if(endThing > i[0] && i[0] >= startThing)
			{
				...
				daText.setFormat(Paths.font("impact.otf"), 12, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
				daText.xAdd = -410;
				daText.borderSize = 1;
	...
	function selectNote(note:Note):Void
	{
		...
		{
			if(note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += Main.ammo[_song.mania];
			for (i in _song.notes[curSec].sectionNotes)
			{
	...
	{
		var noteDataToCheck:Int = note.noteData;
		if(noteDataToCheck > -1 && note.mustPress != _song.notes[curSec].mustHitSection) noteDataToCheck += Main.ammo[_song.mania];
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		{
			_song.notes[curSec].sectionNotes.push([noteStrum, (noteData + Main.ammo[_song.mania]) % (Main.ammo[_song.mania] * 2), noteSus, noteTypeIntMap.get(daType)]);
		}
...
class ChartingState extends MusicBeatState
{
	...
	*
	...
	function reloadGridLayer() {
		...
		if(sectionStartTime(1) <= FlxG.sound.music.length)
		{
			nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE + GRID_SIZE * Main.ammo[_song.mania] * 2, Std.int(GRID_SIZE * 32 * curZoom));
			leHeight = Std.int(gridBG.height + nextGridBG.height);
			foundNextSec = true;
		}
		else nextGridBG = new FlxSprite().makeGraphic(1, 1, FlxColor.TRANSPARENT);
